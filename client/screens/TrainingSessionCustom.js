...
import {ActivityIndicator, Alert, Animated, BackHandler, TouchableOpacity, View} from 'react-native'
import {gql, useMutation} from "@apollo/client";
import {useNavigation, useRoute} from "@react-navigation/native";

const ADD_HISTORY_EXERCISE = gql(`
    mutation CreateHistoryCategory($userExerciseId: String) {
        createHistoryCategory(userExerciseId: $userExerciseId) {
            userId
            userExerciseId
        }
    }
...
export default function TrainingSessionCustom() {
  const {userExerciseId, exercises, duration, restDuration: restTime} = useRoute().params;
  ...
  return (
    <Box className='flex flex-col items-center justify-between h-[80%] w-full bg-gray-100 p-4 rounded-lg'>
      ...
      <VStack className='items-center w-full'>
        ...
        <Text
          className='font-semibold text-3xl text-gray-800 mb-5'>{rest ? "Rest" : exercises[currentExerciseIndex].name}</Text>
        ...
        <HStack className="flex w-[100%] justify-evenly items-center mt-6">
          <Button variant="outline"
                  className="rounded-full border-2 border-green-500 px-3 bg-white shadow-md hover:bg-green-100"
          ...
          <Box className="flex items-center justify-center">
            <TouchableOpacity onPress={handleSkipButton} className="w-20 h-20 p-4 bg-gray-200 rounded-full shadow-lg"
                              disabled={rest}>
