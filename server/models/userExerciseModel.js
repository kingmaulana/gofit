...
class UserExerciseModel {
  ...
  static async findAll(id) {
    ...
    try {
      return await this.collection()
        .aggregate([
          {$match: {userId: ObjectId.createFromHexString(id)}},
          {
            $lookup: {
              from: "exercise",
  ...
  static async create(args, userId) {
    try {
      const userExercise = await this.collection()
        .insertOne({
          name: args.name,
          userId: ObjectId.createFromHexString(userId),
          exerciseId: args.exerciseId.map(id => ObjectId.createFromHexString(id)),
          duration: args.duration,
          restDuration: args.restDuration
        })

      return await this.collection().findOne({_id: userExercise.insertedId})
    } catch (error) {
      throw new Error(error)
    }
  }
  ...
  static async updateExercise(args, userId) {
